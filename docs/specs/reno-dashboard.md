...
- **AC-8**: Blocked complaints, work orders and RKB job rows render as blocked with their citation reachable in one click, and their clock displayed as paused rather than counting down. Asserted per entity type.

  *Previously amended to drop RKB job rows, because no record type could express one. That amendment is withdrawn: `rkb_block` is the ninth record type and exists for this. It carries no `state` field and has no unblocked variant — a block either happened or no record exists — and because every record carries `source_message_id`, an uncited block is unrepresentable rather than rejected. Net realisation can now differ from gross, which it could not before.*
...
- **AC-11**: Charts pass the data-viz gates: categorical hues assigned in fixed order and never cycled, no dual-axis chart, a legend present for two or more series, a table view available on every chart, and a validated palette in both light and dark. `pnpm test:viz` runs the palette validator against the shipped palette and asserts zero FAILs.

  *Amended: light mode only, because there is no dark mode. `DESIGN.md` says so under "Not covered yet", and is explicit that dark steps must be **selected** from these ramps and validated against a dark surface rather than produced by inverting the light ones. Validating a dark palette that no screen uses would be a passing check on nothing. `pnpm test:viz` prints "dark mode: not shipped" rather than passing silently; when dark mode is built, this criterion comes back with it.*
